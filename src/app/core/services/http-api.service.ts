...
export class HttpApiService {
  ...
  private getApplicationIdentifier(): string {
    const appId = 'Basic tokengoeshere';
    if (Object.keys(appId).length === 0)
      return '';
    return appId;
  }
  ...
  public get<TT>(url: string): Observable<TT> {
    const headerJson = new HttpHeaders({ 'authToken': this.getApplicationIdentifier() });
    const options = {
      headers: headerJson,
  ...
  public create<TT>(body: Object, url: string): Observable<TT> {
    const header = {
      'authToken': this.getApplicationIdentifier()
    };
    const options = {
